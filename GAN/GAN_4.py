...
def generator(x_input):
    ...
    def full_connect_layer(name, input, nodes):
        ...
        with tf.name_scope(name):
            weight = tf.Variable(tf.truncated_normal([input_shape[1], nodes], stddev=0.1), name='weight')
            bias = tf.Variable(tf.zeros(nodes), name='bias')
    ...
    def deconv_layer(name, input, kernel_size, output_depth):
        def get_deconv_kernel(name, input_depth, kernel_size, output_depth):
            ...
            return tf.Variable(tf.truncated_normal([kernel_size, kernel_size, output_depth, input_depth], stddev=0.1), name=name)
...
def discriminator(x_input):
    ...
    def conv_layer(name, input, kernel_size, output_depth):
        def get_conv_kernel(name, input_depth, kernel_size, output_depth):
            return tf.Variable(tf.truncated_normal([kernel_size, kernel_size, input_depth, output_depth], stddev=0.1), name=name)
    ...
    with tf.variable_scope('discriminator', reuse=True):
        ...
        conv_layer_2_output = tf.nn.relu(
            tf.layers.batch_normalization(conv_layer('conv_layer_2', pool_layer_1_output, 5, 4), training=True))
...
with tf.name_scope('GAN'):
    ...
    d_vars = [var for var in t_vars if var.name.startswith('d_output')]
    g_vars = [var for var in t_vars if var.name.startswith('g_output')]
